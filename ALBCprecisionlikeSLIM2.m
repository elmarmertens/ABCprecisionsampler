function [llf, CC, QQ, RR1, arows, acols, asortndx, brows, bcols, bsortndx] = ...
    ALBCprecisionlikeSLIM2(aaa,invbbb,ccc,y,x0,invsqrtsig0,CC,QQ,RR1,arows, acols, asortndx, brows, bcols, bsortndx)
...
Ptilde11    = Ptilde(1:N1,1:N1);
Ptilde12    = Ptilde(1:N1,N1+1:NN); 
Ptilde22    = Ptilde(N1+1:NN,N1+1:NN);
% Ptilde21    = Ptilde(N1+1:NN,1:N1);

x1tilde     = RR1 \ (Y - EY);
logdetR11   = 2 * sum(log(abs(diag(RR1)))); % abs since RR1 is output from QR

%% alt
Phat12            = Ptilde12 / chol(Ptilde22);
sqrtinvvarX1tilde = chol(Ptilde11 - Phat12 * Phat12', 'upper');
x1dev             = sqrtinvvarX1tilde * x1tilde;
logdetX11         = 2 * sum(log(diag(sqrtinvvarX1tilde))); 
