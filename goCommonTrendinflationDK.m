%% trend inflation model with state space sampling via DK sampling
...
AA                = zeros(Nstates, Nstates);

% unit root and companion for const trends
AA(ndxbar,ndxbar) = eye(Nbar);
AA(Nbar + Ngap + 1 : end, Nbar + (1 : Ngap * (p-1))) = eye(Ngap * (p-1));

BBsv               = zeros(Nstates, Nshocks,T);

CC             = [ones(N,1) eye(N, Ngap * p)];

State0         = zeros(Nstates, 1);
cholSigma00    = blkdiag(1e4 * eye(Nbar), 10 * eye(Ngap * p));
...
for thisMCMCstep = -burnin : Nmcmc
    ...
    %% filter data
    BBsv(ndxbar,ndxbar,:)       = eye(Nbar) .*  permute(exp(.5 * logSV), [1 3 2]);
    BBsv(ndxgap,ndxgapshocks,:) = repmat(sqrtOMEGAgap, [1 1 T]);
    AA(ndxgap,ndxgapstates)     = PAI;
    
	Ytilde = Yinflation - Y0;
    [States, shockDraws, State0Draws] = abcDisturbanceSmoothingSamplerNaN1draw(AA, BBsv, CCnan, Ytilde, Ynan, State0, cholSigma00, [], [], rndStream);

    StateBar              = States(ndxbar,:);
    shockBar              = shockDraws(ndxbar,:);
    

    StateGap(1:p,:)       = fliplr(reshape(State0Draws(ndxgapstates), N, p))';
    StateGap(p+1:end,:)   = transpose(States(ndxgap,:));
    StateGap              = transpose(Y0) + StateGap;
