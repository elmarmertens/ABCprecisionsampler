...
% alternative implementation with VAR intercept tracked by intercept vector of precision-based sampler (instead of mean adjusted obs vector)
...
rowsX00   = transpose(1 + (1:Ngap)) + (Ngap+1) .* (0 : T - 1);
rowsX00   = (Nbar + Ngap) * p + rowsX00(:);
stackPAI0 = PAI0 .* ones(1,T);
X00       = sparse(rowsX00,1,stackPAI0(:));
...
for thisMCMCstep = -burnin : Nmcmc
    ...
    stackPAI0 = PAI0 .* ones(1,T);
    X00       = sparse(rowsX00,1,stackPAI0(:));
    Xdraw = ALBCprecisionsamplerNaN(aaa,invbbb,ccc,Yinflation,Ynan,X00,invsqrtSigma00,rndStream,...
    ...
    StateGap              = transpose(States(ndxgap, :));
